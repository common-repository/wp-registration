...
class WPR_Profile {
	...
	public function set_user_data( $user_id ) {
		...
		$profile_Id = isset($this->user->form_id) ? $this->user->form_id : '';
        $this->form = new WPR_Form($profile_Id);
  ...
  //      	wp_enqueue_script( 'wpr-bsrp' );
		...
		// frontent file load
	    wp_enqueue_style('wpr-account', WPR_URL."/css/wpr-account.css");
	    wp_enqueue_style('wpr-register', WPR_URL."/css/wpr-register.css");
	...
	function render_profile() {
		...
		$profile_template 	= "profile/public-profile.php";
    ...
    function profile_change_password(){
 	...
 	function delete_user_account() {
 		...
 		// Verify nonce
	    if ( ! isset($_POST['wpr_nonce']) || ! check_ajax_referer('wpr_profile_updating', 'wpr_nonce', false) ) {
	        wp_send_json( array('status' => 'error', 'message' => __('Invalid request. Please try again.', 'wpr')) );
	    }
	
	    // Check if the user is logged in
	    if ( ! is_user_logged_in() ) {
	        wp_send_json( array( 'status' => 'error', 'message' => __('You must be logged in to delete an account.', 'wpr') ) );
	    }
	    ...
	    // Prevent self-deletion (optional, based on your logic)
	    if ( get_current_user_id() !== $user_id ) {
	        wp_send_json( array('status' => 'error', 'message' => __('You cannot delete other users account.', 'wpr')) );
	    }
	    ...
	    if ( $deleted ) {
	        ...
	        $response = array('status' => 'error', 'message' => __('Failed to delete the account.', 'wpr'));
	    }
