...
if( ! defined('ABSPATH' ) ){
    ...
    function wpr_admin_user_column_data( $val, $column_name, $user_id ) {
        ...
        switch ( $column_name ) {
            ...
            case 'user_status':
                ...
                $return = wpr_account_status_change($account_status_val, $user_id);
                break; 
        }
    ...
    function wpr_account_status_change($account_status_val, $user_id) {
        $status_val = $account_status_val === 'active' ? 'active' : 'inactive';
        $status_update = $account_status_val === 'active' ? 'inactive' : 'active';
        
        $icon_class = $account_status_val === 'active' ? 'dashicons-yes' : 'dashicons-no';
    
        $status_url = admin_url("admin-post.php") . "?action=wpr_update_status&userID={$user_id}&statusKey={$status_update}";
    
        $html = '<a href="' . esc_url($status_url) . '" class="dashicons ' . esc_attr($icon_class) . '" title="' . esc_attr($status_val) . '"></a>';
...
*/
    ...
    /*------------------------------------------------------------------------
     This metabox function use to display Multi-Step Form  
    --------------------------------------------------------------------------*/
    function wpr_admin_setting_step_form_metabox(  ){
    
        add_meta_box( 
            'wpr_step_setting',
            __( 'Multi-step Registration' , ' wp-registration'),
            'wpr_admin_setting_step_form',
            'wpr',
            'normal',
            'default'
        );
    ...
    function wpr_admin_setting_step_form(  ){

        // load email template setting
        wpr_load_templates("admin/step.php");
    }
